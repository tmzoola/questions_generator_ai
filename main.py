...
async def generate_api_questions(text: str, question_type: str, question_count: int, question_difficulty: str):
    ...
    if question_type == "multiple choice":
        ...
        for question in parsed_questions:
            ...
            for option in question['options']:
                if not option.startswith('a)') and not option.startswith('b)') and not option.startswith('d)') and not option.startswith('c)'):
    ...
    response_data = {"questions": parsed_questions, "format": format_value, "uuid": uuid.uuid4()}
